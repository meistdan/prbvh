...
template <typename T> 
__device__ __forceinline__ int delta(int i, int j, int n, T * mortonCodes);

template <>
__device__ __forceinline__ int delta<unsigned long long>(int i, int j, int n, unsigned long long * mortonCodes) {
	if (j < 0 || j >= n) return -1;
...
template <>
__device__ __forceinline__ int delta<unsigned int>(int i, int j, int n, unsigned int * mortonCodes) {
	if (j < 0 || j >= n) return -1;
	unsigned long long a = mortonCodes[i];
	unsigned long long b = mortonCodes[j];
	if (a != b) return __clz(a ^ b);
	else return __clz(i ^ j) + sizeof(unsigned long long) * 8;
}
