...
extern "C" __global__ void computeNodeOffsetsBFS(
	const int threads,
	...
	const int taskBegin = taskOffset + threadIndex;
	const int taskEnd = (divCeilLog(numberOfTasks, LOG_WARP_THREADS) << LOG_WARP_THREADS) + taskOffset;
	for (int taskIndex = taskBegin; taskIndex < taskEnd; taskIndex += threads) {
